...
test("validates function type", (t) => {
  ...
  t.true(guard({ callback: () => undefined }));
...
test("extra properties do not cause failure", (t) => {
  const guard = schemaGuard({ name: "string" });
  t.true(guard({ another: 42, extra: true, name: "Alice" }));
});
...
test("combined schema with multiple types", (t) => {
  ...
  t.true(guard({ active: true, age: 30, name: "Alice", tags: ["admin"] }));
  t.true(
    guard({
      active: true,
      age: 30,
  ...
  t.false(guard({ active: true, age: 30, name: "Alice", tags: [1] }));
});
...
test("mixed valid and invalid fields", (t) => {
  const guard = schemaGuard({ age: "number", name: "string" });
  t.false(guard({ age: "thirty", name: "Alice" }));
  t.false(guard({ age: 30, name: 42 }));
});
